...
#     def __init__(self, patch_len, stride, enable_channel_independence=False):
#         super().__init__()
#         # Patching
...
class Patching(nn.Module):
    ...
    def forward(self, x):  # (B, T, C)
        x = rearrange(x, "B T C -> B C T")                 # (B, C, T)
        x = x.unfold(dimension=-1, size=self.patch_len, step=self.stride)  # (B,C,T_p,P)
        if self.enable_channel_independence:
            x = rearrange(x, "B C T_p P -> (B C) T_p P")
        else:
            x = rearrange(x, "B C T_p P -> B T_p (C P)")
        return x